...
param(
    ...
    [ValidateSet('Complete', 'Close', 'Timeout', 'ValidationFailure', 'ProcessHandoff')][string]$Scenario = 'Complete'
)
...
& $module {
    param($Case)
    $script:gdapBrowserProbe = @{ Scenario = $Case; Seconds = 0; Polls = 0; EstsExchanges = 0; PortalValidations = 0; Stopped = $false; Profile = $null; BrowserCloses = 0 }
    ...
    function script:Start-Process {
        ...
        $script:gdapFakeProcess = [pscustomobject]@{ HasExited = ($script:gdapBrowserProbe.Scenario -eq 'ProcessHandoff') }
    ...
    function script:Get-M365BrowserCdpVersion { @{ webSocketDebuggerUrl = 'ws://dedicated-browser.invalid' } }
    function script:Get-M365BrowserPreferredTargetContext {
        $url = if ($script:gdapBrowserProbe.Seconds -ge 36 -and $script:gdapBrowserProbe.Scenario -in @('Complete', 'ValidationFailure', 'ProcessHandoff')) { 'https://admin.cloud.microsoft/' } else { 'https://login.microsoftonline.com/' }
        @{ WebSocketUrl = 'ws://test.invalid'; Url = $url; Title = 'Sign in' }
    }
    function script:Get-M365BrowserCookieJar {
        if ($script:gdapBrowserProbe.Scenario -eq 'Close' -and $script:gdapFakeProcess.HasExited) { throw 'Synthetic closed CDP endpoint' }
        $script:gdapBrowserProbe.Polls++
        [pscustomobject]@{ name = 'ESTSAUTH'; value = 'SYNTHETIC_ESTS'; domain = 'login.microsoftonline.com' }
        if ($script:gdapBrowserProbe.Scenario -in @('Complete', 'ValidationFailure', 'ProcessHandoff') -and $script:gdapBrowserProbe.Seconds -ge 36) {
    ...
    function script:Invoke-M365BrowserCdpCommand {
        param($WebSocketUrl, $Method)
        if ($Method -ne 'Browser.close' -or $WebSocketUrl -ne 'ws://dedicated-browser.invalid') { throw 'Unexpected CDP command' }
        $script:gdapBrowserProbe.BrowserCloses++
    }
...
$expected = switch ($Scenario) {
    ...
    'ProcessHandoff' { '*GDAP_TEST_PORTAL_VALIDATION_BOUNDARY*' }
...
if ($Scenario -in @('Complete', 'ValidationFailure', 'ProcessHandoff') -and ($probe.Polls -ne 3 -or $probe.PortalValidations -ne 1)) {
...
if ($Scenario -eq 'ProcessHandoff' -and $probe.BrowserCloses -ne 1) { throw 'Handoff left the dedicated browser open' }
& $module {
