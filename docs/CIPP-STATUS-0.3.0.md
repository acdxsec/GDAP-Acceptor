# Historical read-only CIPP onboarding connector — 0.3.0 development

> Deployment direction changed: do not configure a CIPP API client secret on
> each workstation. Version 0.3.0 implements direct workstation access, but the
> intended replacement is centrally hosted status access with technician sign-in.
> The replacement is implemented in 0.4.0 source but is not deployed. Leave old local API access
> unconfigured; GDAP acceptance and CIPP webhook-driven onboarding still work.
> The instructions below describe the existing 0.3.0 implementation, not the
> recommended deployment. See [central status setup](CIPP-STATUS.md).
